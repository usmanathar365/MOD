...
<style>
  ...
  .popup-feild {
    ...
    .pop-feild-label [type="radio"]:checked + label:before, [type="radio"]:not(:checked) + label:before {
        width: 20px;
        height: 20px;
    }

    .pop-feild-label [type="radio"]:checked + label:after, [type="radio"]:not(:checked) + label:after {
        width: 10px;
        height: 10px;
        top: 5px;
        left: 5px;
    }
    .pop-feild-label [type="radio"]:checked + label, [type="radio"]:not(:checked) + label {
        padding-left: 25px;
        font-size: 13px;    
    }
</style>
      ...
      <div class="row align-items-center">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            @if($invoice->total_amount_paid != $invoice->total_amount) 
            <a onclick="update()" href="javascript:void(0)">Update</a>
            @endif 
          </ol>
        </div><!-- /.col -->
      </div><!-- /.row -->
          ...
          <div class="row align-items-center">
            <div class="col-sm-5">
              ...
              <div class="invoice-logo" data-toggle="tooltip" data-placement="top" title="Change logo">
                ...
                <img id="logo_img" src="{{ url('/') }}/public/uploads/invoices/{{$invoice->brand_logo}}">
                @else
                <img id="logo_img" src="{{ url('/') }}/public/uploads/brands/{{$invoice->brand->logo}}">
                @endif
            ...
            <div class="col-sm-7">
              <div class="d-flex justify-content-between">
                ...
                <div class="d-1 d-c ml-4">
                  ...
                  <h6 id="brand_name" contenteditable> {{ $invoice->brand_name }} </h6>
                  <h5 id="brand_phone" contenteditable> {{ $invoice->brand_phone }} </h5>
                </div>
                <div class="d-1 d-c">
                  <h6 id="brand_address" contenteditable> {{ $invoice->brand_address }} </h6>
                  <h5><span contenteditable> {{ $invoice->brand_country }} </span></h5>
                </div>
            ...
            <div class="col-sm-3">
              <div class="d-6">
                ...
                <select name="client_id" class="form-control client_id">
                  ...
                  <option {{ ($invoice->client_id == $client->id)?'selected':''  }} value="{{$client->id}}"> {{$client->first_name}} {{$client->last_name}}
                  </option>
                  @endforeach
                  @endif
                </select>
              ...
              <div class="d-flex justify-content-between">
                <div class="d-3">
                  <div class="d-gap mb-5">
                    ...
                    <input value="{{date('Y-m-d', strtotime($invoice->issue_date))}}" type="date" id="issue_date" name="issue_date">
                    <span id="issue_date_err" class="text-danger laravel_validation"></span>
                  </div>
                  <div class="d-gap">
                    <h6 class="custom-label">Due Date</h6>
                    <input type="date" id="due_date" value="{{date('Y-m-d', strtotime($invoice->due_date))}}" name="due_date">
                ...
                <div class="d-4">
                  <div class="d-gap mb-5">
                    ...
                    <input type="text" readonly name="invoice_number" value="{{$invoice->invoice_number}}" placeholder="23832">
                    <span id="invoice_number_err" class="text-danger laravel_validation"></span>
                  </div>
                  <div class="d-gap">
                    <h6 class="custom-label">Reference</h6>
                    <input type="text" name="reference" value="{{$invoice->reference}}" placeholder="Enter Value (e.g. PO#)">
                ...
                <div class="d-5">
                  <div class="d-gap">
                    ...
                    <h5 name="total_amount" id="total_amount">$</h5>
                    ...
                    <div class="d-gap">
                      <h6 class="custom-label">Currency</h6>
                        <select name="currency" selected class="form-control ">
                          ...
                          <option {{($invoice->currency == 'USD' )?'selected':''}} value="USD">USD
                          </option>
                          <option {{($invoice->currency == 'GBP' )?'selected':''}} value="GBP">GBP
                          </option>
                          <option {{($invoice->currency == 'EUR' )?'selected':''}} value="EUR">EUR
                          </option>
                          <option {{($invoice->currency == 'CAD' )?'selected':''}} value="CAD">CAD
                          </option>
                          <option {{($invoice->currency == 'AUD' )?'selected':''}} value="AUD">AUD
                          </option>
                          <option {{($invoice->currency == 'AED' )?'selected':''}} value="AED">AED
                          </option>
                           <option {{($invoice->currency == 'PKR' )?'selected':''}} value="PKR">PKR
                          </option>
                        </select>
                    </div>
                    ...
                    <div class="invoice-lineItem js-invoice-line-item js-line-guid-ember614" data-ebd-id="-trigger">
                      ...
                      <div class="invoice-lineItemCell invoice-lineItemCell--nonNumeric">
                        <div class="js-item-select js-invoice-line-item-name ember-view"><span id="ember616" class="js-type-ahead-control ember-view">
                            <textarea aria-haspopup="listbox" aria-controls="ember616-typeahead" aria-expanded="false" aria-autocomplete="list" role="combobox" @can('invoice-delete') @else readonly @endcan placeholder="Enter an Item Name" class=" form-control ember-text-area typeahead-filter form-unstyledInput js-selected-item ember-view item-name-textarea" aria-label="Item Name" style="overflow: hidden;height: 54px;">{{$item->title}}</textarea>
                            <!---->
                          </span>
                        </div>
                        <div class="invoice-lineItemSecondaryData">
                          <textarea placeholder="Enter an Item Description" @can('invoice-delete') @else readonly @endcan class="ember-text-area form-unstyledInput item-description-textarea  form-control">{{$item->description}}</textarea>
                        </div>
                      </div>
                      <div class="invoice-lineItemCell">
                        <input aria-label="Item Rate (USD)" value="{{(int)$item->rate}}" @can('invoice-delete') @else readonly @endcan placeholder="$0.00" maxlength="9" class="ember-text-field  line-rate form-unstyledInput js-item-rate ember-view" type="number">
                        <div class="invoice-lineItemSecondaryData invoice-addTaxButton link">
                          <div id="ember621" class="ember-view">
                            ...
                            <div class="hoverable">
                              ...
                              @can('invoice-delete')
                              <a onclick="addTaxes().call(this)" href="javascript:void(0)"> Add Taxes</a> @else @endcan
                            </div>
                      ...
                      <div class="invoice-lineItemCell">
                        <div>
                          <div class="ember-view"><input aria-label="Item Quantity" @can('invoice-delete') @else readonly @endcan value="{{(int)$item->quantity}}" maxlength="19" class="line-qty ember-text-field form-unstyledInput js-item-quantity js-ember613-item-quantity ember-view" type="number">
                          </div>
                        </div>
                        <!---->
                      </div>
                      <div class="invoice-lineItemCell js-invoice-line-item-total">
                        <div>
                          <input readonly aria-label="Line Total (USD)" value="{{(int)$item->total}}" placeholder="$0.00" maxlength="9" class=" line-total ember-text-field form-unstyledInput ember-view" type="number">
                        </div>
                      </div>
          ...
          <div class="row">
            <div class="col-12 col-sm-12">
              <div class="d-7 d-gap-1">
                <h6>Notes</h6>
                <input type="text" name="notes" value="{{$invoice->notes}}" placeholder="Enter Notes or bank transfer details (optional)">
            ...
            <div class="col-12 col-sm-12">
              <div class="d-7">
                <h6>Terms</h6>
                <textarea id="terms"  name="terms" placeholder="Enter your terms and conditions (Pro tip: It pays to be polite. FreshBooks invoices that include 'please' and 'thanks' get paid up to 2 days faster.)">
                {{$invoice->terms}}
                </textarea>
        ...
        <div class="col-12 col-sm-3">
          ...
          <div class="card card-s-wrap">
            ...
            <div class="card-body">
              ...
              <div class="{{$method->id}}">
                <p>
                  <input type="radio" value="{{$method->id}}" {{($invoice->payment_method == $method->id)?'checked':''}} id="{{$method->id}}" name="payment_method">
                  <label for="{{$method->id}}">
              ...
              <div class="payment2">
                ...
                <div class="form-group">
                    ...
                    <div class='d-flex m-auto mb-2 '>
                                     <input id="partialPayments" {{($invoice->partial_payments == 1)?'checked':''}} type="checkbox" style="display:block">

                                    <input id="partialPaymentAmount" value ="{{$invoice->partial_payment_amount}}" name="partialPaymentAmount" {{($invoice->partial_payments == 1)?'':'disabled'}}   type="number"  placeholder="$0.00" style="display:block; background-color: #fff; width: 25%;">
                    </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div id="ember1541" class="popover--taxPicker__column ember-view">
          ...
          <div class="popover-section taxesPopover-scroll">
                  ...
                  <td class="js-taxes-popover-checkbox taxesPopover-checkbox">
                    <label class="m-0">
                      <span class="form-input form-input--checkbox js-tax-checkbox is-unchecked ember-view">
                        <input checked class="form-control tax-row-id" aria-label="Select va3" type="checkbox">
...
?>
                            <input aria-label="Tax Rate (Percentage)" value="{{$tax_}}" placeholder="0" maxlength="6" class="form-control tax-row-percentage" type="text" style="overflow: hidden; width: 51px;">
                           ...
                           <div class="container-toggle">
                            ...
                            @else
                                <input id="check" checked class="toggle-check" onclick="onOff(this)" type="checkbox">
                            @endif
                  ...
                  <td class="u-paddingBottom--half u-paddingRight--half taxes-popover-name js-taxes-popover-name">
                    <input aria-label="Tax name" placeholder="Tax name" value="{{$tax->title}}" maxlength="15" class=" tax-row-name form-control " type="text">
                  </td>
                  <td class="u-paddingBottom--half js-taxes-popover-number" colspan="2">
                    <input aria-label="Tax number" placeholder="Tax number" value="{{$tax->number}}" maxlength="25" class=" tax-row-number form-control " type="text">
                  ...
                  <td class="u-paddingBottom--half u-paddingRight--half u-verticalAlign--middle js-taxes-popover-amount">
                    <div class="form-inputIconSplit">
                      <div class="d-flex align-items-center">
                            ...
                            <input aria-label="Tax Rate (Percentage)" value="0" placeholder="0" maxlength="6" class="form-control tax-row-percentage" type="text" style="overflow: hidden; width: 51px;">
          ...
          <div class="modal-footer">
            ...
            <button class="btn btn-success" id="apply-tax"  data-dismiss="modal" type="button">Apply Taxes
            </button>
          </div>
        </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div id="ember1541" class="popover--taxPicker__column ember-view">
        ...
        <div class="mb-2 mt-2">
              ...
              $discount = (float) $invoice->total_discount *100 / (float) $invoice->subtotal_amount;
              }else{
              $discount = 0;
              }
              @endphp
    ...
    <?php
                            if($invoice->discount_type == "percentage"){
                                ...
                                $discount_ = (float) $invoice->total_discount;
                            }
    ...
    ?>
          ...
          <div class="row align-items-center mt-3 mb-3">
            ...
            <div class="col-sm-4 d-flex">
              ...
              <input aria-label="Discount (Percentage)" placeholder="0" maxlength="6" class="form-control " value="{{$discount_}}" id="discount"  type="text">
                           ...
                           <div class="container-toggle">
                            ...
                            @else
                              <input id="discount-type" checked class="toggle-check" onclick="onOff(this)" type="checkbox">
                            @endif
                              <span class="toggle "></span>
                            </label>
                            <div class="on">$</div>
...
<script>
  ...
  amountPaid.innerHTML = "{{ $invoice->total_amount_paid}}";
  var taxes = [];
  var items = [];
  var suggestions = [];
  var suggestionItems = [];
...
<script>
  ...
  function update() {
    ...
    data.total_amount = total.innerHTML;
    data.taxes = taxes;
    ...
    $.ajax(settings).done(function(response) {
      ...
      if (response.status == false) {
        ...
        swal("Invoice Updated!", response.message, "success");


        setTimeout(function() {
          window.location = "{{route('invoices.list')}}";
        }, 2000);
